...
namespace Bussbolag
{
    public partial class Form1 : Form
    {
        ...
        {
            string selectedFrom = listBox4.GetItemText(listBox4.SelectedItem),
               selectedToo = listBox5.GetItemText(listBox5.SelectedItem);

            reseId = 0;

            connection.Open();
           
            cmd.CommandText = "SELECT reseId FROM bussresa WHERE (avgstad='" + selectedFrom + "') AND (stadnamn='" + selectedToo + "')";
            reseId = (int)cmd.ExecuteScalar();
            cmd.ExecuteNonQuery();
            cmd.CommandText = "select pris from bussresa where reseId ='" + reseId + "'";
            pris = (int)cmd.ExecuteScalar();
            cmd.ExecuteNonQuery();
            pris = pris * antal;
            connection.Close();
            textBox6.Text = pris.ToString();
        }
